...
export const AMtoMQ = function(){
    ...
    const AMQsqrt = {input:"sqrt", tag:"msqrt", output:"sqrt", ttype:UNARY},
...
//{input:"color", tag:"mstyle", ttype:BINARY}
    ...
    const AMQnames = []; //list of input symbols
...
// if str[0] is a digit or - return maxsubstring of digits.digits
    ...
    let AMQnestingDepth,AMQpreviousSymbol,AMQcurrentSymbol;
...
//    if (symbol.input == "/") AMQTremoveBrackets(node);
    ...
    let AMQTallowmatrices = true;
    function AMQTparseExpr(str,rightbracket) {
        let symbol, node, result, i, nodeList = [],
          newFrag = '';
...
//    if (AMQnestingDepth > 0) AMQnestingDepth--;
    ...
    return function(str,elid,nomatrices) {
        ...
        let out = AMQTparseExpr(str.replace(/^\s+/g,""),false)[0];
...
*/
export function MQtoAM(tex,display) {
    let nested,lb,rb,isfuncleft,curpos,c,i;
    tex = tex.replace(/\\:/g,' ');
